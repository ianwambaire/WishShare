# Birthday Wishlist Application

This is an iOS mobile application built using Swift and Storyboard that allows users to create personal wishlists, view friends’ birthday wishlists, contribute towards gifts as a group, and receive reminders for upcoming birthdays.

The application focuses on simplifying birthday gift planning by enabling social collaboration, wishlist sharing, and pooled contributions toward gifts.

---

## Application Features

The application supports social interactions between users and provides tools for birthday planning and group gifting.

### User and Social Features
- Create and manage a personal user profile  
- Send friend requests to other users  
- Accept or reject incoming friend requests  
- View and manage a list of friends  

### Birthday Management
- View upcoming birthdays of friends  
- Receive reminders when a friend’s birthday is approaching  

### Wishlist Management
- Create a personal wishlist  
- Add items to a wishlist  
- View friends’ wishlists  

### Group Gifting
- Contribute money towards a gift as a group  
- Track individual contributions from each friend  
- View progress toward achieving the gift goal  

---

## Technology Stack

- Programming Language: Swift  
- User Interface: Storyboard  
- Platform: iOS  
- Architecture: MVC  
- Development Environment: Xcode  

---

## Visuals of the Application

### 1. Authentication and Onboarding

Users can sign up, log in, and access the application features after authentication.

<p align="center">
  <img src="https://github.com/user-attachments/assets/7100cb39-4986-4136-8df5-5982e48481d9" width="220">
  <img src="https://github.com/user-attachments/assets/97fde527-8ce5-4e6e-89d5-a292ded2a751" width="220">
</p>

---

### 2. Home and Birthday Overview

Users can view upcoming birthdays and navigate to friends’ wishlists.

<p align="center">
  <img src="https://github.com/user-attachments/assets/00d0eb61-050c-49e1-9619-395cfde683e5" width="220">
</p>

---

### 3. Wishlist Management

Users can create wishlists, add items, and view wishlist details.

<p align="center">
  <img src="https://github.com/user-attachments/assets/a6101bf4-3496-4162-bae0-1471656dee2e" width="220">
</p>

---

### 4. Group Gifting and Contributions

Friends can contribute towards a gift and track how much each person has paid toward the total goal.

<p align="center">
  <img src="https://github.com/user-attachments/assets/2362ad4f-1725-4ef4-abad-9a7cfa39eb95" width="220">
</p>

---

## Installation

### Clone the Repository

Use GitHub to clone the repository locally.

### Open in Xcode

1. Open the cloned project folder.
2. Double-click the `.xcodeproj` file to open it in Xcode.
3. Select an iOS simulator or a connected physical device.
4. Run the application using **Command + R**.

---

## Project Structure
Controllers
Models
Views
Storyboards
Assets.xcassets
Supporting Files

---

## License

MIT License

---

## Author

Wambaire Ian Nganga


